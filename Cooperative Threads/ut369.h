...
/* Creates a new thread is ready to start running the assigned function.
 *
 * Parameters:
...
/* Terminates the execution of the calling thread. This function should never 
 * return, i.e., the calling thread does not continue running after this call.
...
/* Kill the thread specified by the identifier tid. A killed thread will
 * cease execution of its assigned function. The calling thread will
 * continue to execute normally and receives the return value of 
 * this call. 
 *
 * Parameters:
...
/* Suspends the execution of the calling thread and schedules the thread 
 * identified by tid to run. If the calling thread remains runnable, it is 
...
/* Suspends the execution of the calling thread and schedules another thread 
...
/* Wakes up one or more threads that are suspended in the specified wait queue 
...
/* Blocks the calling thread until the specified thread terminates, optionally 
 ...
 *   reused.
 * - On failure: Returns THREAD_INVALID. Failure can occur for the following 
 *   reasons:

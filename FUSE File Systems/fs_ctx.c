...
bool fs_ctx_init(fs_ctx* fs, void* image, size_t size)
{
    ...
    /** EXFS Superblock is first block on disk, so the pointer to the
     *  superblock is the same as the pointer to the start of the
     *  mmap'd disk image.
     */
    fs->sb = (exfs_superblock*)image;
     ...
     *  image appears to be a valid EXFS file system.
     */
    if (fs->sb->sb_magic != EXFS_MAGIC) {
        return false;
    }

    /** EXFS Inode bitmap pointer
     *  The block number of the inode bitmap is EXFS_IMAP_BLKNUM;
     *  we multiply by the block size to get the offset in bytes from the
     *  start of the mmap'd disk image.
     */
    fs->ibmap = (bitmap_t*)(image + EXFS_IMAP_BLKNUM * EXFS_BLOCK_SIZE);

    /** EXFS Data block bitmap pointer
     *  Similar calculation as inode bitmap.
     */
    fs->dbmap = (bitmap_t*)(image + EXFS_DMAP_BLKNUM * EXFS_BLOCK_SIZE);

    /** EXFS Inode table pointer
     *  Similar calculation as for bitmaps.
     */
    fs->itable = (exfs_inode*)(image + EXFS_ITBL_BLKNUM * EXFS_BLOCK_SIZE);
